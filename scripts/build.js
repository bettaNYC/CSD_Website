...
/* Homepage gallery filters. `id` = the project's `category` field in
   ...
   submenu targets (index.html#residential etc.), applied by site.js on load. */
...
function renderGalleryTile(p, base) {
  ...
  return [
    `    <a class="gallery-tile reveal" href="${base}projects/${esc(p.slug)}.html" data-category="${esc(p.category || "")}"${hideAllAttr} aria-label="${esc(p.title)}">`,
